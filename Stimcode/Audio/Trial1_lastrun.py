...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\maana\\Documents\\GitHub\\TalkingHeads\\New_exp\\Stim code\\Audio\\Trial1_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    trials = data.TrialHandler(nReps=400.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions(audio_file_paths),
    ...
    for thisTrial in trials:
        ...
        sound_1.setSound(sound, secs=3.75, hamming=True)
        ...
        # --- Run Routine "trial" ---
        routineForceEnded = not continueRoutine
        while continueRoutine and routineTimer.getTime() < 5.75:
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            tThisFlipGlobal = win.getFutureFlipTime(clock=None)
            ...
            if cross.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > cross.tStartRefresh + 5.75-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if sound_1.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > sound_1.tStartRefresh + 3.75-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        routineForceEnded = not continueRoutine
        while continueRoutine and routineTimer.getTime() < 8.75:
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            tThisFlipGlobal = win.getFutureFlipTime(clock=None)
